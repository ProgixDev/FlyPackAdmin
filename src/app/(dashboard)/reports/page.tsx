...
export default async function ReportsPage({
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">{reports?.length ?? 0} résultat(s)</p>
      ...
      <div className="mt-6 flex flex-col gap-3">
        {(reports ?? []).map((r) => (
          <div key={r.id} className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  <Link href={`/users/${r.reporter_id}`} className="font-medium text-blue-600 hover:underline">
                    {nameById.get(r.reporter_id) ?? 'Utilisateur'}
                  </Link>{' '}
                  a signalé{' '}
                  <Link href={`/users/${r.reported_user_id}`} className="font-medium text-blue-600 hover:underline">
            ...
            {r.description && <p className="mt-3 text-sm text-slate-600">{r.description}</p>}
          </div>
        ))}
