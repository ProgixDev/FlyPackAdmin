...
export default async function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/users" className="text-sm font-medium text-blue-600 hover:underline">
        ← Utilisateurs
      </Link>

      <div className="mt-3 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900">{profile.full_name || 'Sans nom'}</h1>
      ...
      <section className="mt-8">
        <h2 className="text-sm font-bold text-slate-900">Trajets publiés ({trips?.length ?? 0})</h2>
            ...
            <tbody>
              {(trips ?? []).map((t) => (
                <tr key={t.id} className="border-b border-slate-100 last:border-0">
                  <td className="px-4 py-2.5 text-slate-700">{t.from_city} → {t.to_city}</td>
                  <td className="px-4 py-2.5 text-slate-600">{new Date(t.departure_date).toLocaleDateString('fr-FR')}</td>
                  <td className="px-4 py-2.5 text-slate-600">{t.kilos_max} kg</td>
      ...
      <section className="mt-8">
        <h2 className="text-sm font-bold text-slate-900">Historique des colis envoyés ({bookings?.length ?? 0})</h2>
              ...
              {(bookings ?? []).map((b) => {
                ...
                return (
                  <tr key={b.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-4 py-2.5 text-slate-600">{b.kilos} kg</td>
                    <td className="px-4 py-2.5 text-slate-600">{b.status}</td>
                    <td className="px-4 py-2.5 text-slate-600">{new Date(b.created_at).toLocaleDateString('fr-FR')}</td>
                  </tr>
                );
              })}
      ...
      <section className="mt-8">
        <h2 className="text-sm font-bold text-slate-900">Signalements reçus ({reportsAgainst?.length ?? 0})</h2>
        <div className="mt-3 flex flex-col gap-2">
          ...
          {(!reportsAgainst || reportsAgainst.length === 0) && (
            <p className="text-sm text-slate-400">Aucun signalement reçu.</p>
          )}
        </div>
      </section>
    </div>
